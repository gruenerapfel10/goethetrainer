import { UIMessage, streamText, appendResponseMessages } from 'ai';

import { auth } from '@/app/(auth)/auth';
import { generateUUID, getMostRecentUserMessage, messagesWithoutFiles } from '@/lib/utils';
import { getChatById, saveChat, saveMessages, listCsvTables, executeCsvQuery } from '@/lib/db/queries';
...
import { initializeRegularTools } from '@/lib/ai/agent-registry';
...
export const streamCsvAgent = async (
    ...
    if (!session || !session.user || !session.user.id) {
        console.error('streamCsvAgent: Unauthorized - No session or user ID');
    ...
    if (!userMessage) {
        console.error('streamCsvAgent: No user message found in messages array');
...
LEAST TOOL USAGE AS POSSIBLE, PLEASE.
  ...
  `;
                ...
                const regularTools = initializeRegularTools('csv-agent-v2', {
                    dataStream,
