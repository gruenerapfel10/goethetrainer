...
export function GoetheHeader({ sectionLabel = 'LESEN', levelLabel }: GoetheHeaderProps) {
  const sectionText = sectionLabel.toUpperCase();
  const levelText = levelLabel?.toUpperCase();
  return (
    <div className="w-96 mx-auto">
      {/* Exam Header */}
      <div className="border border-border flex">
        <div className="flex-[2] bg-secondary py-1 px-3 flex items-center justify-start">
          <h1 className="text-xs font-bold text-secondary-foreground">
            {levelText ? `GOETHE-ZERTIFIKAT ${levelText}` : 'GOETHE-ZERTIFIKAT'}
          </h1>
        </div>
