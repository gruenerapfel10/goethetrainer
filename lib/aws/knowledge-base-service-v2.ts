...
import {
  ...
  getAccessToken
} from '../sharepoint/sharepoint-service-v2';
...
export async function syncSharepointToS3AndFlagForBedrock(operationId?: number): Promise<{
  ...
  console.log(`[SP Sync Phase 1] Received ${changes.length} changes from SharePoint.${initialSyncPerformedByRecursive ? ' (Initial sync was recursive full scan)' : ''}`);
  ...
  for (const change of changes) {
    ...
    if (changes.length > 0 && (processedCount % logInterval === 0 || processedCount === changes.length || changes.length < logInterval)) {
      ...
      console.log(`[SP Sync Phase 1] Progress: ${progressMessage}`);
      if (operationId && updateSystemOperation) {
        await updateSystemOperation(operationId, operationStatusEnum.enumValues.find(s => s === 'UPLOADING_TO_S3')!, {
  ...
  console.log(`[SP Sync Phase 1] Finished. S3 Attempts: ${s3UploadsAttempted}, Fails: ${s3UploadsFailed}, DB Create/Update Flags: ${dbRecordsUpdatedOrCreated}, DB Delete Flags: ${dbRecordsMarkedForDeletion}, Errors: ${errorsInS3Sync}`);
...
async function autoCleanupStuckDeletionFiles(): Promise<void> {
  try {
    console.log("[Auto Cleanup] Checking for stuck deletion files...");
    ...
    if (stuckFiles.length === 0) {
      console.log("[Auto Cleanup] No stuck deletion files found.");
      return;
    }

    console.log(`[Auto Cleanup] Found ${stuckFiles.length} stuck deletion files older than 24 hours. Attempting cleanup...`);
    ...
    for (const file of stuckFiles) {
      try {
        ...
        try {
          ...
          console.log(`[Auto Cleanup] Deleted S3 object: ${file.s3Key}`);
        } catch (s3Error: any) {
          if (s3Error.name === 'NoSuchKey' || s3Error.$metadata?.httpStatusCode === 404) {
            console.log(`[Auto Cleanup] S3 object ${file.s3Key} already deleted`);
          } else {
        ...
        console.log(`[Auto Cleanup] Cleaned up stuck file: ${file.s3Key}`);
        cleanedUp++;
    ...
    if (cleanedUp > 0) {
      console.log(`[Auto Cleanup] Successfully cleaned up ${cleanedUp} stuck deletion files.`);
    }
...
export async function processPendingBedrockOperations(operationId?: number): Promise<BedrockProcessingResults> {
  console.log("[Bedrock Processor] Starting new processing cycle...");
  if (operationId && updateSystemOperation) {
  ...
  console.log(`[Bedrock Processor] Full processing cycle finished after ${results.totalLoops} loops. Final Results: ${JSON.stringify(results)}.`);
  return results;
}
...
export async function uploadAndRegisterManualFiles(
  ...
  console.log(`[Manual Upload] Batch complete. ${message}`);
