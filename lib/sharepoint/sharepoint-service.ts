...
export async function getSharePointChanges(driveIdToSync: string = DRIVE_ID, targetPathKey: string = TARGET_PATH_TO_SYNC_ENV): Promise<{ changes: SharePointChange[]; newDeltaLinkPersisted: boolean; actualItemsFoundThisRun: number; initialSyncPerformedByRecursive?: boolean }> {
  ...
  try {
    do {
      ...
      /*
      if (items.length > 0) {
          console.log("SharePoint Sync DEBUG: First few raw items on this page (up to 5):");
          items.slice(0, 5).forEach((rawItem, idx) => {
              console.log(`  Item ${idx}: Name='${rawItem.name}', ID='${rawItem.id}', IsFile=${!!rawItem.file}, IsFolder=${!!rawItem.folder}, IsDeleted=${!!rawItem.deleted}, Path=${rawItem.parentReference?.path}, cTag=${rawItem.cTag}, eTag=${rawItem.eTag}, downloadUrl=${rawItem['@microsoft.graph.downloadUrl'] || rawItem.downloadUrl}`);
          });
      }
      */
      for (const item of items) {
